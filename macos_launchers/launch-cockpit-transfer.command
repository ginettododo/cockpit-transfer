...
UV_BIN="$(find_uv || true)"
if [ -n "$UV_BIN" ]; then
  echo "Launching with uv (automatic Python 3.12)..."
  exec "$UV_BIN" run --python 3.12 main.pyw "$@"
fi

PYTHON_BIN="$(find_python || true)"
if [ -z "$PYTHON_BIN" ]; then
  echo "ERROR: Python 3.11+ not found or not working."
  echo "Standard Apple Python (3.9.6) is too old."
  echo "Use 'uv' or download Python 3.12 from python.org to continue."
  exit 1
fi
