...
# Supporto a 'uv' per gestire Python in autonomia (3.11+)
find_uv() {
...
if [ $# -eq 0 ]; then
  echo "INFO: Cerco l'ultimo ZIP in Downloads per l'importazione..."
fi

UV_BIN="$(find_uv || true)"
if [ -n "$UV_BIN" ]; then
  echo "Importazione rapida con uv (Python 3.12+ automatico)..."
